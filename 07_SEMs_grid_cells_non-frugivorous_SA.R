## METADATA ====================================================================================
# SCRIPT: 07_SEMs_grid_cells_non-frugivorous_SA.R
...
# DESCRIPTION:
# Here, we evaluate whether the positive effects of frugivore FRic/SRic on Annonaceae FRic/SRic 
# in our finer spatial resolution analyses (i.e., with grid cells with about 110-km resolution with 
# Behrmann equal area projection) could have resulted from factors unrelated to frugivory (i.e., a Type I error). 
# To this end, we repeated the global SEMs, but using only species of non-frugivorous birds and 
# mammals (i.e., species with 0% of fruits in their diet). We expected that the absence of frugivory-related 
# interactions should lead to a negative or absence of relationship between mammal or bird FRic/SRic 
# and Annonaceae FRic/SRic in the SEMs.
...
### Load data ####
df_f0 <- read.csv("Data/sem_f0_grid")
...
# Fitted model
mod <- 'annon_fric ~ annon_sric_sqrt + brd_fric_sqrt + annual_prec
brd_fric_sqrt ~ brd_sric_sqrt + annual_temp + temp_season + annual_prec + prec_season
mml_fric_sqrt ~ mml_sric_sqrt + annual_prec + prec_season
annon_sric_sqrt ~ brd_sric_sqrt + temp_season + annual_prec + prec_season
brd_sric_sqrt ~ annual_temp + temp_season
mml_sric_sqrt ~ annual_temp + temp_season + annual_prec
brd_sric_sqrt ~~ mml_sric_sqrt 
mml_fric_sqrt ~~ brd_sric_sqrt
mml_fric_sqrt ~~ mml_sric_sqrt
brd_fric_sqrt ~~ annon_sric_sqrt
mml_fric_sqrt ~~ annon_sric_sqrt 
annon_fric ~~ mml_sric_sqrt
